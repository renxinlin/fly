...
void ANTO_polling(void) //轮询扫描上位机端口
{
	...
	switch(status)
	{
		...
		case 2:
			{
				if(*(uint8_t*)&ANTO_Recived_flag == 0) status = 1;//上位机的发过来的数据都被处理了，则返回专心的发送数据到上位机
...
{ 	
	...
	if((USART1->SR & USART_IT_RXNE))
	{
		...
		case 0:
			{
				if(ReciveBuffer[0]==(int8_t)0xAA) count++;
			}
			break;
		case 1:	
			{
				if(ReciveBuffer[1]==(int8_t)0xAF) count++;
				else count = 0;
			}
			break;
		default:
			{
				...
				else
				{
					...
					if(CheckSum == ReciveBuffer[count])  //if the data calculate sum equal to the  final data from PC.
					{
						...
						CheckSend[6] = 0;
						for(i=0;i<6;i++) CheckSend[6] += CheckSend[i]; 
